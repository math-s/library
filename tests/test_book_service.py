...
def mock_google_books_repository():
    with patch('app.repositories.google_books_repository.GoogleBooksRepository') as mock:
        mock_instance = mock.return_value
        mock_instance.search_books = AsyncMock(return_value=[
            GoogleBook(
                id="test_id_1",
                title="Test Book 1",
                authors=["Test Author 1"],
        ...
        mock_instance.get_book_by_isbn = AsyncMock(return_value=GoogleBook(
            id="test_id_1",
            title="Test Book 1",
            authors=["Test Author 1"],
            description="Test Description 1",
        ...
        mock_instance.get_books_by_author = AsyncMock(return_value=[
            GoogleBook(
                id="test_id_1",
                title="Test Book 1",
                authors=["Test Author 1"],
...
async def test_search_google_books(book_service, mock_google_books_repository):
    ...
    mock_google_books_repository.search_books.assert_called_once_with("python programming", 10)

@pytest.mark.asyncio
async def test_get_books_by_author_from_google(book_service, mock_google_books_repository):
    ...
    mock_google_books_repository.get_books_by_author.assert_called_once_with("Test Author", 10) 